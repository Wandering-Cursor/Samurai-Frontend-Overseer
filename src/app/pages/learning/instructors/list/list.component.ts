...
export class ListComponent {
  ...
  loadProjects(page: number = this.currentPage): void {
    this.apiService.getTeacherProjects(page, this.pageSize, this.teacherId)
      .subscribe({
        next: (data) => {
          this.projects = data;
  ...
  getStudentNames(accountLinks: any[]): string {
    return accountLinks.map(link => {
      const details = link.account_details;
