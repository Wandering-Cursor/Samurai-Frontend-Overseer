...
import { Observable, ObservableNotification, map } from 'rxjs';
...
getTeacherStats(): Observable<TeacherStats[]> {
  return this.http.get<TeacherStatsApiResponse>(`${GlobalComponent.API_URL}projects/stats/teachers`).pipe(
    map(response => response.items)
  );
}


getTeacherProjects(page: number = 1, pageSize: number = 10, accountId?: string, facultyId?: string, name?: string): Observable<TeachersProject[]> {
  let params = new HttpParams()
  ...
  if (facultyId) {
    params = params.set('faculty_id', facultyId);
  }
  if (name) {
    params = params.set('name', name);
  }

  return this.http.get<TeachersProjectApiResponse>(`${GlobalComponent.API_URL}projects/projects`, { params }).pipe(
    map(response => response.content)
  );
}
